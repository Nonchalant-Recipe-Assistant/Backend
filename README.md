...
Приложение поддерживает полнотекстовый поиск данных (рецептов) с возможностью сложной фильтрации и сортировки.

**Endpoint:** `GET /search/data`

**Параметры запроса:**
*   `q` (string, optional): Строка полнотекстового поиска. Ищет совпадения (case-insensitive) в полях `title` и `description`.
*   `date_from` (date, YYYY-MM-DD): Фильтр. Показать записи, созданные *после* этой даты.
*   `date_to` (date, YYYY-MM-DD): Фильтр. Показать записи, созданные *до* этой даты.
*   `category` (string): Фильтр по категории (например, "Italian", "Vegan").
*   `sort_by` (string): Поле для сортировки. Возможные значения: `created_at` (по умолчанию), `title`.
*   `sort_order` (string): Направление сортировки. Возможные значения: `desc` (по умолчанию), `asc`.
